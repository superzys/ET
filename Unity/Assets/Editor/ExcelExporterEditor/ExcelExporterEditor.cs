...
public class ExcelExporterEditor : EditorWindow
{
    ...
    /// <param name="exportDir"></param>
    /// <param name="isNeedAdd"> 客户端需要手动添加到 工程文件</param>
	private void ExportClass(string fileName, string exportDir, bool isNeedAdd = false)
    {
        ...
        using (StreamWriter sw = new StreamWriter(txt))
        {
            ...
            sb.Append("\t[ETModel.Config(ETModel.AppType.Client)]\n");
    ...
    private void ExportAll(string exportDir,bool isServer)
    {
        string md5File = Path.Combine(ExcelPath, isServer?"md5Server.txt": "md5.txt");
...
//                if (fieldType != "Int" && fieldType != "float" && fieldType != "long" && fieldType != "bool" && fieldType != "string")
//                {
//                    temStr = $"\"{fieldName}\":{fieldValue}";
//                }
//                else
//                {
                    temStr = $"\"{fieldName}\":{Convert(fieldType, fieldValue)}";
//                }
                sb.Append(temStr);
            }
            sb.Append("}");
